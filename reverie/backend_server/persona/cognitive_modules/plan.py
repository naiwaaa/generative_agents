...
def generate_hourly_schedule(persona, wake_up_hour):
    ...
    n_m1_hourly_compressed_ = []
    prev = None
    prev_count = 0
    for i in n_m1_activity:
        if i != prev:
            prev_count = 1
            n_m1_hourly_compressed_ += [[i, prev_count]]
            prev = i
        elif n_m1_hourly_compressed_:
            n_m1_hourly_compressed_[-1][1] += 1

    # Step 2. Expand to min scale (from hour scale)
    # [['sleeping', 360], ['waking up and starting her morning routine', 60],
    # ['eating breakfast', 60],..
    n_m1_hourly_compressed = []
    for task, duration in n_m1_hourly_compressed_:
...
def _determine_action(persona, maze):
    ...
    def determine_decomp(act_desp, act_dura):
        ...
        if "sleeping" in act_desp or "asleep" in act_desp or "in bed" in act_desp:
            return False
        if "sleep" in act_desp or "bed" in act_desp:
            if act_dura > 60:
                return False
        return True
