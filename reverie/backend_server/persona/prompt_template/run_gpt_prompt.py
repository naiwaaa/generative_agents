...
def run_gpt_prompt_daily_plan(persona, wake_up_hour, test_input=None, verbose=False):
    ...
    def create_prompt_input(persona, wake_up_hour, test_input=None):
        ...
        prompt_input += [f"{wake_up_hour!s}:00 am"]
        return prompt_input

    def __func_clean_up(gpt_response, prompt=""):
        cr = []
        cr_ = gpt_response.split(")")
        for i in cr_:
            if i[-1].isdigit():
                i = i[:-1].strip()
                if i[-1] == "." or i[-1] == ",":
...
def run_gpt_prompt_generate_hourly_schedule(
    ...
    def create_prompt_input(
        ...
        for count, i in enumerate(persona.scratch.daily_req):
            intermission_str += f"{count + 1!s}) {i}, "
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ...
        for count, i in enumerate(temp):
            if count != 0:
                cr_ += [" ".join([j.strip() for j in i.split(" ")][3:])]
            else:
                cr_ += [i]
        for count, i in enumerate(cr_):
...
def run_gpt_prompt_new_decomp_schedule(
    ...
    def __func_validate(gpt_response, prompt=""):
        try:
            ...
            x = [
                datetime.datetime.strptime(i.strip(), "%H:%M %p") for i in x.split(" to ")
            ]
        ...
        if init_persona.a_mem.seq_chat:
            for i in init_persona.a_mem.seq_chat:
                if i.object == target_persona.scratch.name:
                    v1 = int(
                        (init_persona.scratch.curr_time - i.created).total_seconds() / 60
                    )
                    prev_convo_insert += (
                        f"{v1!s} minutes ago, they had the following conversation.\n"
                    )
        ...
        if persona.a_mem.seq_chat:
            for i in persona.a_mem.seq_chat:
                if i.object == target_persona.scratch.name:
                    ...
                    prev_convo_insert += f"{v1!s} minutes ago, {persona.scratch.name} and {target_persona.scratch.name} were already {i.description} This context takes place after that conversation."
                    break
        if prev_convo_insert == "\n":
            prev_convo_insert = ""
        if persona.a_mem.seq_chat:
