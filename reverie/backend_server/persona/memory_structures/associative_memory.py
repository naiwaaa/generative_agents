...
class AssociativeMemory:
    ...
    def add_event(
        ...
        node_type = "event"
        node_id = f"node_{node_count!s}"
        depth = 0

        # Node type specific clean up.
        if "(" in description:
            description = (
                " ".join(description.split()[:3]) + " " + description.split("(")[-1][:-1]
            )
    ...
    def get_summarized_latest_events(self, retention):
        ret_set = set()
        ret_set.update(e_node.spo_summary() for e_node in self.seq_event[:retention])
        return ret_set
