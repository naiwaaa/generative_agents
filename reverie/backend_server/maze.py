...
class Maze:
    def __init__(self, maze_name):
        ...
        wb_ = blocks_folder + "/world_blocks.csv"
        wb_rows = read_file_to_list(wb_, header=False)
        wb = wb_rows[0][-1]

        sb = blocks_folder + "/sector_blocks.csv"
        sb_rows = read_file_to_list(sb, header=False)
        sb_dict = dict()
        for i in sb_rows:
            sb_dict[i[0]] = i[-1]

        ab = blocks_folder + "/arena_blocks.csv"
        ab_rows = read_file_to_list(ab, header=False)
        ab_dict = dict()
        for i in ab_rows:
            ab_dict[i[0]] = i[-1]

        gob = blocks_folder + "/game_object_blocks.csv"
        gob_rows = read_file_to_list(gob, header=False)
        gob_dict = dict()
        for i in gob_rows:
            gob_dict[i[0]] = i[-1]

        slb = blocks_folder + "/spawning_location_blocks.csv"
        slb_rows = read_file_to_list(slb, header=False)
        slb_dict = dict()
        for i in slb_rows:
            slb_dict[i[0]] = i[-1]
        ...
        cm = maze_folder + "/collision_maze.csv"
        collision_maze_raw = read_file_to_list(cm, header=False)[0]
        sm = maze_folder + "/sector_maze.csv"
        sector_maze_raw = read_file_to_list(sm, header=False)[0]
        am = maze_folder + "/arena_maze.csv"
        arena_maze_raw = read_file_to_list(am, header=False)[0]
        gom = maze_folder + "/game_object_maze.csv"
        game_object_maze_raw = read_file_to_list(gom, header=False)[0]
        slm = maze_folder + "/spawning_location_maze.csv"
        spawning_location_maze_raw = read_file_to_list(slm, header=False)[0]
    ...
    def get_nearby_tiles(self, tile, vision_r):
        ...
        """
        left_end = 0
        left_end = max(tile[0] - vision_r, left_end)

        right_end = self.maze_width - 1
        right_end = min(tile[0] + vision_r + 1, right_end)

        bottom_end = self.maze_height - 1
        bottom_end = min(tile[1] + vision_r + 1, bottom_end)

        top_end = 0
        top_end = max(tile[1] - vision_r, top_end)
